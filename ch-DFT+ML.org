...
:END:
# Design parameters other than composition were held constant.
# The composition parameter poses serious challenges to optimization due to the combinatorial scaling experienced in the space as more candidates for each site is introduced.
  
# This 14-dimensional vector is a sufficient descriptor of a perovskites.
# To improve regression, 36 additional predictors derived from linear combinations of compositions and elemental properties obtained from the trusted Mendeleev databases are examined.
...
# Perovskite semiconductors offer opportunities to reduce cost and environmental impact as well as increase the performance of devices ranging from superconductors and computational q-bits to light emitting diodes and solar cells.
...
:END:
# Set The Stage: What work is done? How does this connect to it?
...
** DONE Multiple Fidelity Dataset
:LOGBOOK:
...
It is known the octahedral arrangement of perovskites is most relevant to their electronic structure, nevertheless, a strong understanding of the influence of chemical composition on performance will continue to be a priority as it is expected to aid in [[Feature Engineering][Feature Engineering]].
...
A detailed analysis of this combined hybrid organic-inorganic and purely inorganic HaP DFT dataset is covered in a prior article by [cite/text:@yang-2022-high-throug] and in [[DFT Details]].
Naturally, the statistics obtained from each fidelity vary (figure ref:fig:bg_dist).
This is the primary challenge we will address with the categorically dimensioned multi-fidelity models discussed in [[Methods]].

#+begin_src jupyter-python :post wrap(*this*, c="label:fig:bg_dist Variability in band gaps at each fidelity")
...
[[file:./.ob-jupyter/e43c69abaab32a55a294c19aa5cdde4a867cdce8.svg]]
:end:
...
:END:
The simulations for each level of theory are performed on some number of members to a fixed subset of the total 37785 compositions that can be combinatorially generated in a 2x2x2 perovskite supercell when allowing /at most/ single-site alloying with our 14 constituent candidates for 3 sites (table ref:tbl:site_tbl).
...
The simulated experiments are performed on some subset (table ref:tbl:LoTs) of ~500, exclusively pseudo-cubic, \(\ce{ABX_3}\) supercells obtained by geometry optimization of modified structure files [cite:@pilania-2016-machin-learn] originally obtained from the Computational Materials Repository[fn:2].
...
- 36 dense site-averaged property space computed as a linear combination of composition vectors and measured elemental properties [cite:@mentel-2014]
...
Thiprocedure in demonstrated in an online notebook by [cite/text:@manganaris-2022-mrs-comput] hosted on the Purdue NanoHUB.
...
#+attr_latex: :width 300pt
#+CAPTION: 
[[file:./.ob-jupyter/afacec3b693eab0abeba71b7b157548cabf0e90d.svg]]
:end:
...
#+attr_latex: :width 300pt
#+CAPTION: 
[[file:./.ob-jupyter/85566ad2104fdd918f8748e90247977ea57f2913.svg]]
:end:
...
#+attr_latex: :width 300pt
#+CAPTION: 
[[file:./.ob-jupyter/52b2f8da20b8039dc06e5fc3935c270210c7c9b5.svg]]
:end:
...
[[file:./.ob-jupyter/dc863520dccc5f4cefbddf3272f3866b5d78ae59.svg]]
:end:
...
#+begin_src jupyter-python
  ...
  print(
      "bg\mbox{ [eV]} = " + mbox_ammend(typeset_sisso_model(cpipe).replace(" kJpermol", ""))
  )
#+end_src
...
#+attr_latex: :width 300pt
#+CAPTION: 
[[file:./.ob-jupyter/1cfbeb7d17e75e535304290461c8be5b27e5146d.svg]]
:end:
...
#+attr_latex: :width 300pt
#+CAPTION: 
[[file:./.ob-jupyter/377183c6393fbd0cf4a9641563bd03c7421bc455.svg]]
:end:
...
#+begin_src jupyter-python :post wrap(*this*, w="450pt :options inkscapeformat=png, inkscapedpi=300", c="label:fig:pairplots SIS-based model predictions vs true values at multiple fidelities")
...
[[file:./.ob-jupyter/2ecd3e9f2764e7415d29b9a5564477a1c896acea.svg]]
:end:
...
[[file:./.ob-jupyter/86ea07ec49cfd9a1052cd7c3bf12cf13c3ed9f9b.svg]]
:end:
...
Namely the Goldschmidt's tolerance, the octahedral tolerance, and the tolerance proposed by [cite/text:@bartel-2019-new-toler].

These cuts trimmed the data set from ~40000 points to a subset of 3247 viable candidates.
