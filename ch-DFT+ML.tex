...
\label{sec:org1a56607}
\section*{Introduction}
\label{sec:org496076a}
\subsection*{Multi-Fidelity Learning}
\label{sec:orga9dbc4c}
...
Graph convolutional neural networks can power more accurate structure-target predictions at multiple fidelities by performing Multi-Task Learning (MTL).
...
\label{sec:org396475c}
\subsection*{Perovskite Band Gaps}
\label{sec:orge8c3339}
...
It is known the octahedral arrangement of perovskites is most relevant to their electronic structure, nevertheless, a strong understanding of the influence of chemical composition on performance will continue to be a priority as it is expected to aid in \hyperref[sec:org655d0ae]{Feature Engineering}.
...
A detailed analysis of this combined hybrid organic-inorganic and purely inorganic HaP DFT dataset is covered in a prior article by and in \hyperref[sec:org678df10]{DFT Details}.
Naturally, the statistics obtained from each fidelity vary (figure \ref{fig:bg_dist}).
This is the primary challenge we will address with the categorically dimensioned multi-fidelity models discussed in \hyperref[sec:org9db1314]{Methods}.
...
See \hyperref[sec:org9db1314]{Methods} for details on how these categories were utilized in model development.
...
See \hyperref[sec:org8e7cd33]{Discussion}.
...
Every fitted model is subsequently tested against the cross-validation test sets and a suite of regression scoring metrics are applied to each member category simultaneously using a custom SciKit-learn score adapter\footnote{\url{https://github.com/PanayotisManganaris/yogi}\label{org20a6745}}.
The grid search is then narrowed to a high performance quadrant of the search space by the model evaluator based on recommendations made by a simple entropy minimization algorithm\textsuperscript{\ref{org20a6745}}.
...
\label{sec:org15b5ddc}
For \(\alpha\) total A-site constituents represented in the whole database, \(\beta\) total B-site constituents, and \(\gamma\) total X-site constituents, we provide a Python tool\footnote{\url{https://github.com/PanayotisManganaris/cmcl}\label{org98e8235}} which robustly coverts the composition string of each data point into a \(\alpha + \beta + \gamma\) dimensional composition vector.
...
\begin{itemize}
\item 14 sparse composition vectors extracted from chemical formula using \texttt{cmcl}\textsuperscript{\ref{org98e8235}}
\item 36 dense site-averaged property space computed as a linear combination of composition vectors and measured elemental properties
...
Thiprocedure in demonstrated in an online notebook by hosted on the Purdue NanoHUB.

\subsection*{Feature Engineering}
\label{sec:org655d0ae}
There has been success in creating analytical expressions for perovskite properties, particularly lattice parameters. In an attempt to find an analytical predictor for band gap we employ the Sure Independence Screening and Sparsifying Operator (SISSO). 
...
SISSO is run for our dataset on the same partitioning scheme used by the previous models via an SciKit-learn compliant interface\footnote{\url{https://github.com/PanayotisManganaris/pysisso}} extensively modified from the original Matgenix\footnote{\url{https://github.com/Matgenix/pysisso}} code.
...
\section*{Results}
\label{sec:orgaa5c868}
\subsection*{Best Models on Raw Domain}
\label{sec:orga82e6e0}
...
We expected this to solve the problems inherent to the raw \hyperref[sec:org15b5ddc]{Featurization of Chemistries}.
...
\begin{figure}[htbp]
\centering
\includesvg[inkscapeformat=png, inkscapedpi=300,width=450pt]{./.ob-jupyter/2ecd3e9f2764e7415d29b9a5564477a1c896acea}
\caption{\label{fig:pairplots} SIS-based model predictions vs true values at multiple fidelities}
\end{figure}

\section*{Discussion}
\label{sec:org8e7cd33}
\subsection*{SHAP Analysis of Domain}
\label{sec:org4b07ee1}
...
\begin{figure}[htbp]
\centering
\includesvg[inkscapeformat=png, inkscapedpi=300,width=400pt]{./.ob-jupyter/86ea07ec49cfd9a1052cd7c3bf12cf13c3ed9f9b}
...
Namely the Goldschmidt's tolerance, the octahedral tolerance, and the tolerance proposed by.
