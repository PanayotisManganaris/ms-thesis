...
\label{sec:orgcca3cc9}
\section*{Multi-Fidelity Learning}
\label{sec:org2b99fe9}
...
\label{sec:org7186356}
\subsection*{Perovskite Band Gaps}
\label{sec:org0e392f5}
...
It is known the octahedral arrangement of perovskites is most relevant to their electronic structure, nevertheless, a strong understanding of the influence of chemical composition on performance will continue to be a priority as it is expected to aid in \hyperref[sec:orge3f6e66]{Feature Engineering}.
...
A detailed analysis of this combined hybrid organic-inorganic and purely inorganic HaP DFT dataset is covered in a prior article by \textcite{yang-2022-high-throug} and in \hyperref[sec:orgc3e672b]{DFT Details}.
Naturally, the statistics obtained from each fidelity vary (figure \ref{fig:bg_dist}).
This is the primary challenge we will address with the categorically dimensioned multi-fidelity models discussed in \hyperref[sec:orga249b79]{Methods}.
...
See \hyperref[sec:orga249b79]{Methods} for details on how these categories were utilized in model development.
...
See \hyperref[sec:orgb3e6292]{Discussion}.
...
Every fitted model is subsequently tested against the cross-validation test sets and a suite of regression scoring metrics are applied to each member category simultaneously using a custom SciKit-learn score adapter\footnote{\url{https://github.com/PanayotisManganaris/yogi}\label{org20893dc}}.
The grid search is then narrowed to a high performance quadrant of the search space by the model evaluator based on recommendations made by a simple entropy minimization algorithm\textsuperscript{\ref{org20893dc}}.
...
\section*{Methods}
\label{sec:orga249b79}
\subsection*{DFT Details}
\label{sec:orgc3e672b}
...
\label{sec:orgc40720e}
For \(\alpha\) total A-site constituents represented in the whole database, \(\beta\) total B-site constituents, and \(\gamma\) total X-site constituents, we provide a Python tool\footnote{\url{https://github.com/PanayotisManganaris/cmcl}\label{orge540f96}} which robustly coverts the composition string of each data point into a \(\alpha + \beta + \gamma\) dimensional composition vector.
...
\begin{itemize}
\item 14 sparse composition vectors extracted from chemical formula using \texttt{cmcl}\textsuperscript{\ref{orge540f96}}
...
\section*{Results}
\label{sec:org356dc64}
\subsection*{Best Models on Raw Domain}
\label{sec:orgb8c35dc}
...
We expected this to solve the problems inherent to the raw \hyperref[sec:orgc40720e]{Featurization of Chemistries}.
...
\section*{Discussion}
\label{sec:orgb3e6292}
\subsection*{SHAP Analysis of Domain}
\label{sec:org161b4a9}
