...
\label{sec:org84da4e8}
\section{Multi-Fidelity Learning}
\label{sec:orgf6966b4}
...
\label{sec:orgc2ad881}
\subsection{Perovskite Band Gaps}
\label{sec:org1210311}
...
It is known the octahedral arrangement of perovskites is most relevant to their electronic structure, nevertheless, a strong understanding of the influence of chemical composition on performance will continue to be a priority as it is expected to aid in \hyperref[sec:orgfe2ba05]{Feature Engineering}.
...
A detailed analysis of this combined hybrid organic-inorganic and purely inorganic HaP DFT dataset is covered in a prior article by \textcite{yang-2022-high-throug} and in \ref{sec:orgf936a11}.
Naturally, the statistics obtained from each fidelity vary (figure \ref{fig:bg_dist}).
This is the primary challenge we will address with the categorically dimensioned multi-fidelity models discussed in \ref{sec:orgc13e663}.
...
See \ref{sec:orgc13e663} for details on how these categories were utilized in model development.
...
Every fitted model is subsequently tested against the cross-validation test sets and a suite of regression scoring metrics are applied to each member category simultaneously using a custom SciKit-learn score adapter\footnote{\url{https://github.com/PanayotisManganaris/yogi}\label{org4b2ecd4}}.
The grid search is then narrowed to a high performance quadrant of the search space by the model evaluator based on recommendations made by a simple entropy minimization algorithm\textsuperscript{\ref{org4b2ecd4}}.
...
\section{Methods}
\label{sec:orgc13e663}
\subsection{DFT Details}
\label{sec:orgf936a11}
...
\begin{itemize}
\item 14 sparse composition vectors extracted from chemical formula using \texttt{cmcl}\textsuperscript{\ref{orgf794d03}}
...
\section{Results}
\label{sec:org279cff0}
\subsection{Best Models on Raw Domain}
\label{sec:orgfb8a927}
...
We expected this to solve the problems inherent to the raw \ref{sec:org648bc86}.
...
\section{Discussion}
\label{sec:org1afd58f}
\subsection{SHAP Analysis of Domain}
\label{sec:orge41e51e}
