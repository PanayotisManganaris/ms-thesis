...
\section{Background}
\label{sec:orgeae8cfd}
\begin{frame}[label={sec:org78cb07c}]{Photovoltaic (PV) Power Generation}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org2d51b6f}]{Rapid Rise of Perovskite Photovoltaics}
\begin{columns}
...
\begin{frame}[label={sec:orgec541f3}]{What's a Perovskite?}
...
\begin{frame}[label={sec:org2c37f3b}]{Halide Perovskites}
\begin{columns}
\begin{column}{0.6\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org62f11cc}]{Perovskite Formability}
\begin{itemize}
...
\begin{frame}[label={sec:orgb45cd7f}]{My Thesis Research Overview}
...
\begin{frame}[label={sec:org4c5808f}]{Design Goals in \alert{14-Dimensional} Composition Space}
\begin{columns}
\begin{column}{0.5\columnwidth}
...
\begin{frame}[label={sec:orga187512}]{Sample of 14-Dimensional Composition Space}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org37317de}]{Collecting Data is Hard}
\begin{columns}
\begin{column}{0.7\columnwidth}
...
\label{sec:org0467859}
\begin{frame}[label={sec:org8781975}]{Density Functional Theory (DFT)}
\begin{itemize}
...
\begin{frame}[label={sec:org1ea4d94}]{DFT Calculation of Perovskite Band Gaps in Literature}
\begin{center}
...
\begin{frame}[label={sec:orga1c5730}]{Machine Learning for Property Prediction}
\begin{columns}
\begin{column}{0.7\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org6c35669}]{Data Driven Framework for Perovskite Discovery}
\begin{center}
...
\begin{frame}[label={sec:org047a1a0}]{Random Forest Regression (RFR)}
\begin{columns}
\begin{column}{0.6\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org41b4fba}]{Gaussian Process Regression (GPR)}
\begin{columns}
\begin{column}{0.6\columnwidth}
...
\begin{frame}[label={sec:org1616da6}]{Sure-Independence Screening and Sparsifying Operator (SISSO)}
...
\section{Data}
\label{sec:org5679424}
\begin{frame}[label={sec:org9cd8fa4}]{Effect of DFT Functionals Levels of Theory}
...
\begin{frame}[label={sec:org61946bc}]{Spectroscopic Limited Maximum Efficiency (SLME)}
...
\begin{frame}[label={sec:org682728c}]{Absorbance Relates to Efficiency via Band Gap in Our Data}
...
\begin{frame}[label={sec:org55c89b3},fragile]{Describing a Halide Perovskite}
 \begin{columns}
\begin{column}{0.4\columnwidth}
...
\begin{frame}[label={sec:org994a4d0}]{40000 Composition Vectors projected to 2D}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org5b2540f}]{Constituent Properties}
...
\begin{frame}[label={sec:org54c85cc}]{Correlating Band Gap with 36 Dimensional Property vector}
...
\label{sec:org760629b}
\begin{frame}[label={sec:org0cc4785}]{Multi-fidelity Machine Learning}
\[
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge540a36},fragile]{Tuning Multi-Fidelity Model Performance by Selecting Hyper-parameters}
 \begin{columns}
\begin{column}{0.7\columnwidth}
\begin{center}
...
\section{Models}
\label{sec:org8b3067b}
\begin{frame}[label={sec:org7050703}]{Model Training Method}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{enumerate}
...
\begin{frame}[label={sec:org5f802e0}]{Test 282 Band Gap Predictions}
...
\begin{frame}[label={sec:org0713d50}]{Compare RMSE of different subsets}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{center}
\begin{tabular}{lrrr}
...
\begin{frame}[label={sec:org0da69b5}]{SISSO Expression}
\begin{align*}
...
\begin{frame}[label={sec:org2fe561a}]{Shapely Additive Explanation (SHAP)}
...
\begin{frame}[label={sec:orgfccc3fd}]{SHAP Analyze RFR}
\begin{center}
...
\label{sec:org56db488}
\begin{frame}[label={sec:org2e1241d}]{Make Predictions on \alert{40000} Hypothetical Compounds}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org9c2951d}]{Screening Yields}
\begin{columns}
\begin{column}{0.3\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org8e9c803}]{Screened Constituent Frequencies}
...
\begin{frame}[label={sec:org75a8b41}]{Interpreting t-SNE clusters}
\begin{columns}
\begin{column}{0.5\columnwidth}
...
\label{sec:org56a0c5e}
\begin{frame}[label={sec:org3cf8c34},fragile]{Review of Work}
 \begin{itemize}
...
\begin{frame}[label={sec:orgaf3486f},fragile]{Publications Presentations and Other Contributions}
 \begin{columns}
...
\begin{frame}[label={sec:org20fe34a}]{Conclusions}
\begin{itemize}
...
\section{Credit}
\label{sec:org1828670}
\begin{frame}[label={sec:orga4ccb49}]{Acknowledgements}
\begin{columns}
\begin{column}{0.6\columnwidth}
...
\end{frame}
\section{Bib}
\label{sec:org5577f93}
\begin{frame}[allowframebreaks]{References}
\AtNextBibliography{\tiny}
\printbibliography
\end{frame}
\appendix
\begin{frame}[label={sec:orgcca9953}]{Data Pre-Processing}
\begin{center}
...
\end{frame}
\begin{frame}[label={sec:org3d4b1a0}]{DFT Functionals Effect Band Structure Calculation in HaPs}
\begin{itemize}
\item D3
\item SOC
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb83023d}]{SHAP Analyze GPR}
\begin{center}
