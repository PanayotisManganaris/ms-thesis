...
\begin{block}{Approaching DFT Simulation}
...
\begin{block}{Levels of Theory (LoT) used}
\begin{description}
\item[{Perdew-Burke-Erzenhof (PBE)}] Generalized Gradient Approximation
\item[{Heyd-Scuzeria-Erzenhof (HSE)}] Hybrid GGA and Hartree-Folk Theory
\item[{HSE(SOC)}] Hybrid GGA and Hartree-Folk Theory + corrections for \(\ce{Pb}\) etc.
\item[{HSE(SOC) on PBE relaxed structure}] compromise, using HSE(SOC) only for electronic
\end{description}
\end{block}
\end{frame}
...
index & cat\_\textsubscript{LoT}\textsubscript{EXP} & cat\_\textsubscript{LoT}\textsubscript{HSE} & cat\_\textsubscript{LoT}\textsubscript{HSE}(SOC) & cat\_\textsubscript{LoT}\textsubscript{HSE}-PBErel(SOC) & cat\_\textsubscript{LoT}\textsubscript{PBE}\\[0pt]
\hline
1373 & 0 & 0 & 0 & 1 & 0\\[0pt]
1420 & 0 & 0 & 0 & 1 & 0\\[0pt]
887 & 1 & 0 & 0 & 0 & 0\\[0pt]
...
\label{sec:org0875c50}
\begin{frame}[label={sec:orgbe8b3aa},fragile]{Review of Work}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd8888fb}]{Conclusions}
\begin{itemize}
...
\begin{frame}[label={sec:orgc86571d}]{Acknowledgements}
\begin{columns}
\begin{column}{0.6\columnwidth}
