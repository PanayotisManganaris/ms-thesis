...
\section{Background}
\label{sec:org1ea9ec8}
\begin{frame}[label={sec:orgff7dbcd}]{Photovoltaic (PV) Power Generation}
\begin{columns}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org9aa2600}]{Rapid Rise of Perovskite Photovoltaics}
\begin{columns}
...
\begin{frame}[label={sec:org669c324}]{What's a Perovskite?}
...
\begin{frame}[label={sec:orge96ee82}]{Halide Perovskites}
\begin{columns}
\begin{column}{0.6\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org1f752b6}]{Perovskite Formability}
\begin{itemize}
...
\begin{frame}[label={sec:org8c37292}]{My Thesis Research Overview}
...
\begin{frame}[label={sec:org1142847}]{Design Goals in \alert{14-Dimensional} Composition Space}
\begin{columns}
\begin{column}{0.5\columnwidth}
...
\begin{frame}[label={sec:org6f76ec8}]{Sample of 14-Dimensional Composition Space}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orge88eefb}]{Collecting Data is Hard}
\begin{columns}
\begin{column}{0.7\columnwidth}
...
\label{sec:orgfe9bf16}
\begin{frame}[label={sec:org27cacbc}]{Density Functional Theory (DFT)}
\begin{itemize}
...
\begin{frame}[label={sec:org98e7acf}]{DFT Calculation of Perovskite Band Gaps in Literature}
\begin{center}
...
\begin{frame}[label={sec:orgb8641ea}]{Machine Learning for Property Prediction}
\begin{columns}
\begin{column}{0.7\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org0bc3a75}]{Data Driven Framework for Perovskite Discovery}
\begin{center}
...
\section{Data}
\label{sec:org754e95e}
\begin{frame}[label={sec:org0cdbe15}]{Effect of DFT Functionals Levels of Theory}
...
\begin{frame}[label={sec:org4061c94}]{Spectroscopic Limited Maximum Efficiency (SLME)}
...
\begin{frame}[label={sec:org4f46923}]{Absorbance Relates to Efficiency via Band Gap in Our Data}
...
\begin{frame}[label={sec:org98ced3e},fragile]{Describing a Halide Perovskite}
 \begin{columns}
\begin{column}{0.4\columnwidth}
...
\begin{frame}[label={sec:orgd5d06c6}]{40000 Composition Vectors projected to 2D}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org42799fd}]{Constituent Properties}
...
\begin{frame}[label={sec:org4f8078c}]{Correlating Band Gap with 36 Dimensional Property vector}
...
\label{sec:orgfd3faa3}
\begin{frame}[label={sec:org5424766}]{Multi-fidelity Machine Learning}
\[
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgeca94fd}]{Random Forest Regression (RFR)}
\begin{columns}
\begin{column}{0.6\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:orgbe2efae}]{Gaussian Process Regression (GPR)}
\begin{columns}
\begin{column}{0.6\columnwidth}
...
\begin{frame}[label={sec:orgaa09a84}]{Sure-Independence Screening and Sparsifying Operator (SISSO)}
...
\begin{frame}[label={sec:orgabbd27d},fragile]{Tuning Multi-Fidelity Model Performance by Selecting Hyper-parameters}
 \begin{columns}
\begin{column}{0.7\columnwidth}
\begin{center}
...
\section{Models}
\label{sec:orge0e4d1c}
\begin{frame}[label={sec:orga06aa57}]{Model Training Method}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{enumerate}
...
\begin{frame}[label={sec:org0790db9}]{Test 282 Band Gap Predictions}
...
\begin{frame}[label={sec:org051cc19}]{Compare RMSE of different subsets}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{center}
\begin{tabular}{lrrr}
...
\begin{frame}[label={sec:org62721aa}]{SISSO Expression}
\begin{align*}
...
\begin{frame}[label={sec:org046056f}]{Shapely Additive Explanation (SHAP)}
...
\begin{frame}[label={sec:org36d97b1}]{SHAP Analyze RFR}
\begin{center}
...
\label{sec:orgbc1e279}
\begin{frame}[label={sec:orgdf89b38}]{Make Predictions on \alert{40000} Hypothetical Compounds}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org6acd904}]{Screening Yields}
\begin{columns}
\begin{column}{0.3\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org97433ef}]{Screened Constituent Frequencies}
...
\begin{frame}[label={sec:org39433c8}]{Interpreting t-SNE clusters}
\begin{columns}
\begin{column}{0.5\columnwidth}
...
\label{sec:org4a7b379}
\begin{frame}[label={sec:org2e5abfc},fragile]{Review of Work}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd53bd88},fragile]{Publications Presentations and Other Contributions}
 \begin{columns}
...
\begin{frame}[label={sec:org1add661}]{Conclusions}
\begin{itemize}
...
\section{Credit}
\label{sec:org4e5807f}
\begin{frame}[label={sec:orgb829edf}]{Acknowledgements}
\begin{columns}
\begin{column}{0.6\columnwidth}
...
\end{frame}
\section{Bib}
\label{sec:orga60d676}
\begin{frame}[allowframebreaks]{References}
\AtNextBibliography{\tiny}
\printbibliography
\end{frame}
\appendix
\begin{frame}[label={sec:org1232fec}]{Data Pre-Processing}
\begin{center}
...
\end{frame}
\begin{frame}[label={sec:org17aafe9}]{DFT Functionals Effect Band Structure Calculation in HaPs}
\begin{itemize}
\item D3
\item SOC
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org12fd4df}]{SHAP Analyze GPR}
\begin{center}
