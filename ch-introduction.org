...
The most recent work presented in this dissertation is being submitted for publication as [cite/full:@manganaris-2023-multi-fidel]. 
Publications from earlier work leading to this dissertation include
[cite/full:@yang-2023-high-throug],
and [cite/full:@manganaris-2022-mrs-comput].
...
** Design Goals in Perovskite Absorbers
...
Mixing at the A-site has been shown to improve formability [cite:@zhang-2019-perov-photov], while B-site and X-site mixing can tune and optimize band gap and therefore the maximum optical wavelength absorbable by the semiconductor.
...
[[file:./.ob-jupyter/9f2a6b3e88bd87edad16a8293ec22e2f0ce74780.svg]]
:end:
...
# It has featured in HT-DFT processing tutorials presented at multiple Materials Research Society (MRS) meetings for the benefit of hundreds of researchers.
...
:LOGBOOK:
- State "DONE"       from "TODO"       [2023-06-01 Thu 13:42]
:END:
...
A recent tolerance factor proposed by [cite/text:@bartel-2019-new-toler] is defined as expression \ref{eq:b}.
...
** Thesis Overview
In chapter \ref{sec:org44ac04c}, I review the work we did in [cite/title:@yang-2023-high-throug][cite:@yang-2023-high-throug] to create a multi-fidelity dataset of perovskite band gaps.
...
# This proof is extended to show that every electron configuration corresponds to a unique wave function.
due to the nuclei.
...
[cite:@kresse-1996-effic-iterat;@kresse-1993-ab-initio]
employing projector-augmented-wave (PAW) pseudo-potentials.
[cite:@kresse-1996-effic-ab;@kresse-1999-from-ultras;@kresse-1994-norm-conser]
...
Each HaP composition was simulated in a \(2\times{}2\times{}2\) supercell, which allowed A and B-site mixing to be performed in discrete 1/8^{th} fractions of the total site occupancy, and X-site mixing in 1/24^{th} fractions, though for simplicity, we restricted X-site mixing to fractions of 3/24.
...
These four measures E_{bg}^{PBE}, E_{bg}^{HSE}, E_{bg}^{HSE(SOC)}, and E_{bg}^{HSE-PBErel(SOC)} populated the multi-fidelity dataset I used for my work.
...
[cite:@yu-2012-ident-poten p.1]
Experimental data [cite:@almora-2020-devic-perfor] broadly agrees with PBE simulation, so the range of 1 to 2 \unit{\electronvolt} was justified (see figure ref:fig:slme).
...
[[file:./.ob-jupyter/087a658e6bb000a199d1fd4552bd9bde2db00444.svg]]
:end:
...
[[file:./.ob-jupyter/8fb481208df6e81245038b7eaa4a261669d515f5.svg]]
:end:
