\ProvidesFile{thesis.tex}[2023-06-13 PurdueThesis thesis.tex file]
...
%
%  Overleaf runs latexmk automatically.  Click "Recompile" one final
%  time after making all changes to make sure everything is done.
%
%  If you are using latexmk manually use
%      latexmk -lualatex thesis
%  to recompile your thesis.  Type
%      latexmk -f -g -lualatex thesis
%  one final time after making all changes before turning in your thesis.        
%
...
% For example, if \H works in the text of your thesis but not in the
% bibliography let \ZZH be a synonym for the current value of \H.
% This is done here so we can use \ZZH for a Hungarian double acute
% accent in Paul Erdos' name in the bibliography.  \H gets redefined
% somewhere below so it won't work in bibliographies (it still works
% fine in text though).  If you won't be using Hungarian double acute
% accents in a bibliography, you can put '%' before the next line to
% make it a comment.
%
% "\oe" has the same problem and solution.
\let\ZZH=\H
% \let\ZZoe=\oe
...
% Test \begin{chapteracknowledgement} ... \end{chapteracknowledgement}
\include{ch-test-chapteracknowledgement}
