...
** Tuning Model Performance by Selecting Hyper-parameters          :B_frame:
:PROPERTIES:
:BEAMER_env: frame
:END:
*** col                                                             :BMCOL:
...
*** nine scores                                             :B_block:BMCOL:
:PROPERTIES:
:BEAMER_col: 0.3
:BEAMER_env: block
:END:
=Yogi= tools for concurrently scoring estimators.
...
** DONE Model Training Method                                      :B_frame:
:PROPERTIES:
:BEAMER_env: frame
:END:
:LOGBOOK:
- State "DONE"       from "NEXT"       [2023-06-08 Thu 10:05]
...
*** RFR Learning Curves                                             :BMCOL:
:PROPERTIES:
:BEAMER_col: 0.4
:END:

#+begin_src jupyter-python
  lc = pd.read_csv(".//LearningCurves/rfr_t_bg.csv", index_col=0)
#+end_src
...
#+begin_src jupyter-python :post wrap(*this*, w="140pt")
  mask = lc.score.str.contains('rmse')
  p = px.line(lc[mask], x='train_sizes', y='valuemean', error_y='valuestd',
              facet_col='score', facet_col_wrap=2, color='partition',
              height=900, width=600, 
              facet_row_spacing=0.06,
              facet_col_spacing=0.1,
              title="RFR Learning Curves")
  p.update_layout(font_family='arial narrow', font_size=15,
                  hovermode='x')
  p.update_yaxes(
      matches=None, showticklabels=True,
      title_font_size=30, tickfont_size=24
  )
  p.update_xaxes(
      title_font_size=30, tickfont_size=24
  )
  p.update_layout(
      legend = dict(itemsizing='constant', orientation='h', font_size=30, y=-0.1),
      margin = dict(r=0),
      title_font_size=30
  )
...
#+attr_latex: :width 140pt
[[file:./.ob-jupyter/fefcc9045a4b90480643b1f76250f805a1260a1f.svg]]
:end:
