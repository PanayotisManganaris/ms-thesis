...
:BEAMER_env: frame
:END:
*** Approaching DFT Simulation                                    :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:
...
*** Find Ground State                                             :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:

\[
E(n) &= \hat{T}(n) + \hat{U}(n) + \hat{V}(n)
\]

\(U(n)\) approximations account for varying levels of quantum theory 

Spin Orbit Coupling (SOC) Accounts for relativistic effects near heavy nuclei

*** Levels of Theory (LoT) used                                   :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:
- Perdew-Burke-Erzenhof (PBE) :: Generalized Gradient Approximation
- Heyd-Scuzeria-Erzenhof (HSE) :: Hybrid GGA and Hartree-Folk Theory
- HSE(SOC) :: Hybrid GGA and Hartree-Folk Theory + corrections for \(\ce{Pb}\) etc.
- HSE(SOC) on PBE relaxed structure :: compromise, using HSE(SOC) only for electronic
...
** TODO Learning from Multi-fidelity dataset                       :B_frame:
:PROPERTIES:
:BEAMER_env: frame
:END:
...
#+RESULTS:
:results:
| index | cat__LoT_EXP | cat__LoT_HSE | cat__LoT_HSE(SOC) | cat__LoT_HSE-PBErel(SOC) | cat__LoT_PBE |
|-------+--------------+--------------+-------------------+--------------------------+--------------|
|  1373 |            0 |            0 |                 0 |                        1 |            0 |
|  1420 |            0 |            0 |                 0 |                        1 |            0 |
|   887 |            1 |            0 |                 0 |                        0 |            0 |
